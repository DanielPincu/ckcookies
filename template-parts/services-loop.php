...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 items-center justify-center container mx-auto md:px-10 pt-10 pb-32 gap-8">
    ...
    if ($query->have_posts()) :
        while ($query->have_posts()) : $query->the_post();
            ...
            $shop_link = get_permalink( wc_get_page_id( 'shop' ) ); // Get WooCommerce Shop page URL
            ?>
            
            <!-- Wrap the whole service item in a link -->
            <a href="<?php echo esc_url($shop_link); ?>" class="relative light h-72 border-4 border-pink-600 rounded-3xl hover:text-[#FDF7EC] duration-300">
                ...
                <div class="absolute z-40 -bottom-10 left-1/2 transform -translate-x-1/2 border-2 border-[#FDF7EC] rounded-full w-32 h-32 bg-pink-500">
                    <?php if ($service_icon): ?>
